...
function IdentitasAnak() {
    ...
    const [Balitas, setBalitas] = useState2([]);
 
  useEffect(() => {
    getBalitas();
  }, []);
 
  const getBalitas = async () => {
    const response = await axios.get("http://localhost:5000/Balitas");
    setBalitas(response.data);
  };
    return (
        <>
                ...
                <div className="containerIsi w-full p-4 sm:pl-32 mt-0 sm:mt-10 mb-20 mx-auto sm:ml-52 sm:mr-0">
                    ...
                    <div className='content-center mt-20 w-full '>
                        <div className=" rounded sm:overflow-hidden shadow-lg w-full sm:max-w-xl m-auto bg-white py-5 rounded-b-3xl rounded-t-3xl">
                            ...
                            {Balitas.map((Balita) => (
                            <table className='table-auto  w-full sm:m-auto border-separate border-spacing-5  left-0' key={Balita.id}>
                                <tr>
                                    <td colSpan="2">
                                        <label className='text-base text-blue-600'>Nama Anak</label><br />
                                        <input className="bg-slate-200 rounded-sm outline-0 h-5 px-5 py-5 rounded-t-xl rounded-b-xl  w-full mb-2" readOnly value={Balita.nama} />
                                    </td>
                                </tr>
                                <tr>
                                    <td colSpan="2">
                                        <label className='text-base text-blue-600'>NIK Anak</label><br />
                                        <input className="bg-slate-200 rounded-sm outline-0 h-5 px-5 py-5 rounded-t-xl rounded-b-xl  w-full mb-2 text-black" readOnly value={Balita.nik_anak}/>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
